...
def dashboard():
    ...
    if period not in ('today','yesterday','7d','this_month','last_month',''): period=''
    ...
    # SUBSTRING match use kiya hai (exact match nahi) — isse chhota punctuation/
    # spacing farak bhi (jaise "3DP-01P-639." ya extra suffix) merge ho jayega.
    BANGALORE_ALIAS_KEYWORDS = ["SHAIK GAOUS", "P1S_2", "P1S-2", "3DP-01P-639", "BENGALURU 3D PRINTER"]
    def disp_printer(city, printer):
        raw = str(printer or "Unknown").strip()
        if city == "Bangalore":
            norm = re.sub(r'\s+', ' ', raw).upper()
            for kw in BANGALORE_ALIAS_KEYWORDS:
                if kw in norm:
                    return "Bengaluru 3D Printer"
        return raw

    # ACTIVE DAY DEFINITION (manager's concern, confirmed correct):
    # `date` column is set ONCE at sync time from the print's START timestamp
    # (see sync_prints() -> "date": st_ist.strftime(...)), never from end_time.
    # So a print that starts at night and finishes at ~2am the next calendar
    # day is entirely attributed to the START day. It does NOT create a false
    # "active day" entry for the next day just because it happened to finish
    # a couple hours after midnight. This applies everywhere date is grouped
    # (dashboard, deep analytics, city page, daily breakdown) since they all
    # read the same stored `date` column.
    ...
    stats=defaultdict(lambda:{"model":"","total":0,"completed":0,"cancelled":0,
        ...
        "today_n":0,"yday_n":0,"week_n":0,"month_n":0,"lm_n":0,
        "today_ok":0,"today_ca":0,"yday_ok":0,"yday_ca":0,"week_ok":0,"week_ca":0,
        "month_ok":0,"month_ca":0,"lm_ok":0,"lm_ca":0})
    for r in grp:
        e=stats[(r["city"], disp_printer(r["city"], r["printer"]))]
        if r["model"] and not e["model"]: e["model"]=r["model"]
        h=float(r["amin"] or 0)/60.0; act=int(r["act"] or 0); d=r["date"]; n=r["total"]
        ok=r["completed"]; ca=r["cancelled"]
        e["total"]+=n; e["completed"]+=ok; e["cancelled"]+=ca
        e["all_h"]+=h; e["all_d"]+=act
        if d==today_s: e["today_h"]+=h; e["today_d"]+=act; e["today_n"]+=n; e["today_ok"]+=ok; e["today_ca"]+=ca
        if d==yday_s:  e["yday_h"]+=h;  e["yday_d"]+=act;  e["yday_n"]+=n;  e["yday_ok"]+=ok;  e["yday_ca"]+=ca
        if week_s<=d<=today_s:      e["week_h"]+=h;  e["week_d"]+=act;  e["week_n"]+=n;  e["week_ok"]+=ok;  e["week_ca"]+=ca
        if month_s<=d<=today_s:     e["month_h"]+=h; e["month_d"]+=act; e["month_n"]+=n; e["month_ok"]+=ok; e["month_ca"]+=ca
        if lm_start_s<=d<=lm_end_s: e["lm_h"]+=h;    e["lm_d"]+=act;    e["lm_n"]+=n;    e["lm_ok"]+=ok;    e["lm_ca"]+=ca

    def av(h,d): return round(h/d,1) if d else 0.0

    PERIOD_LABELS = {'':'All Time','today':'Today','yesterday':'Yesterday','7d':'Last 7 Days',
        'this_month':'This Month','last_month':'Last Month'}
    PERIOD_H_KEY = {'today':'today_h','yesterday':'yday_h','7d':'week_h','this_month':'month_h','last_month':'lm_h'}
    PERIOD_N_KEY = {'today':'today_n','yesterday':'yday_n','7d':'week_n','this_month':'month_n','last_month':'lm_n'}
    PERIOD_OK_KEY = {'today':'today_ok','yesterday':'yday_ok','7d':'week_ok','this_month':'month_ok','last_month':'lm_ok'}
    PERIOD_CA_KEY = {'today':'today_ca','yesterday':'yday_ca','7d':'week_ca','this_month':'month_ca','last_month':'lm_ca'}
    PERIOD_D_KEY = {'today':'today_d','yesterday':'yday_d','7d':'week_d','this_month':'month_d','last_month':'lm_d'}
    ...
    for c in CITIES:
        ...
        for key in machs:
            ...
            hours_rows.append({"city":city,"machine":printer,"model":e["model"],
                ...
                "sel_hours": av(e["all_h"],e["all_d"]) if not period else av(e[PERIOD_H_KEY[period]], e[PERIOD_D_KEY[period]])})
            sr=round(e["completed"]/e["total"]*100,1) if e["total"] else 0
            if period:
                p_total=e[PERIOD_N_KEY[period]]; p_ok=e[PERIOD_OK_KEY[period]]; p_ca=e[PERIOD_CA_KEY[period]]
                p_sr=round(p_ok/p_total*100,1) if p_total else 0
            else:
                p_total=e["total"]; p_ok=e["completed"]; p_ca=e["cancelled"]; p_sr=sr
            prod_rows.append({"city":city,"machine":printer,"model":e["model"],
                "today_n":e["today_n"],"yday_n":e["yday_n"],"week_n":e["week_n"],
                "month_n":e["month_n"],"lm_n":e["lm_n"],
                ...
                "sel_total":p_total,"sel_ok":p_ok,"sel_ca":p_ca,"sel_sr":p_sr})
    ...
    # OVERALL blended Avg Cost + Avg TAT — ek hi number, sab city/machine/material
    # milake (Vishal ne bola: per-machine/per-period table nahi chahiye, sirf
    # 2 simple summary numbers, top par KPI card ki tarah).
    overall_cost_sum = sum(e["all_c"] for e in ct.values())
    overall_cost_cnt = sum(e["all_n"] for e in ct.values())
    overall_dur_sum  = sum(e["all_dur"] for e in ct.values())
    overall_avg_cost = avc(overall_cost_sum, overall_cost_cnt)
    overall_avg_tat  = fmt_tat(overall_dur_sum/overall_cost_cnt) if overall_cost_cnt else "-"

    # Top 5 Parts — All Cities, All Time (seedha dashboard par, Deep Analytics
    # me jaane ki zaroorat nahi). Prints count ke hisab se sabse zyada bane
    # parts, sath me har part ka apna avg cost (Completed parts par, material
    # ke sahi rate se) aur avg TAT.
    part_rows_raw = db.execute("""SELECT part_name, material, material_g, duration_min, status
        FROM prints""").fetchall()
    pstats = defaultdict(lambda: {"cnt":0,"completed":0,"matg":0.0,
        "dur_sum":0,"dur_cnt":0,"cost_sum":0.0,"cost_cnt":0})
    for r in part_rows_raw:
        pn = (r["part_name"] or "").strip()
        if not pn: continue
        e = pstats[pn]; e["cnt"] += 1
        matg = float(r["material_g"] or 0); e["matg"] += matg
        dm = r["duration_min"] or 0
        if dm > 0: e["dur_sum"] += dm; e["dur_cnt"] += 1
        if r["status"] == "Completed":
            e["completed"] += 1
            e["cost_sum"] += part_cost_inr(matg, r["material"]); e["cost_cnt"] += 1

    top_parts_sorted = sorted(pstats.items(), key=lambda x: -x[1]["cnt"])[:5]
    top_parts_rows = []
    for rank,(pn, e) in enumerate(top_parts_sorted, start=1):
        top_parts_rows.append({
            "rank": rank, "part": pn, "prints": e["cnt"], "success": e["completed"],
            "matg": round(e["matg"],1),
            "avg_tat": fmt_tat(e["dur_sum"]/e["dur_cnt"]) if e["dur_cnt"] else "-",
            "avg_cost": round(e["cost_sum"]/e["cost_cnt"]) if e["cost_cnt"] else 0,
        })
    ...
    return render_template('dashboard.html', today=today_s, total=total, last_sync=last_sync,
        hours_rows=hours_rows, prod_rows=prod_rows,
        overall_avg_cost=overall_avg_cost, overall_avg_tat=overall_avg_tat,
        top_parts_rows=top_parts_rows, period=period, period_label=PERIOD_LABELS[period],
...
def deep_analytics():
    ...
    # Top parts — ab har part ka apna avg cost (material-wise sahi rate se)
    # aur avg TAT bhi. Cost sirf Completed prints par (waste material count
    # nahi hota part ke "cost" me), TAT sirf duration_min>0 wali rows par.
    prow_data = db.execute(f"""SELECT part_name, material, material_g, duration_min, status
        FROM prints WHERE {where}""", params).fetchall()
    pstats = defaultdict(lambda: {"cnt":0,"completed":0,"matg":0.0,
        "dur_sum":0,"dur_cnt":0,"cost_sum":0.0,"cost_cnt":0})
    for r in prow_data:
        pn = (r["part_name"] or "").strip()
    ...
    top_parts = sorted(pstats.items(), key=lambda x: -x[1]["cnt"])[:10]
    maxp = max([v["cnt"] for _,v in top_parts] or [1])
    ptab = ""
    for pn, e in top_parts:
        pct = round(e["cnt"]/maxp*100)
        avg_cost = round(e["cost_sum"]/e["cost_cnt"]) if e["cost_cnt"] else 0
        avg_tat = fmt_tat(e["dur_sum"]/e["dur_cnt"]) if e["dur_cnt"] else "-"
        pn_disp = _html.escape(pn[:55])
        ptab += (f"<tr><td title='{_html.escape(pn)}'>{pn_disp}</td>"
            f"<td class='mono'><b>{e['cnt']}</b></td><td class='mono g'>{e['completed']}</td>"
            f"<td class='mono'>{round(e['matg'],1)}g</td><td class='mono' style='color:#7c3aed;font-weight:700'>{avg_tat}</td>"
            f"<td class='mono' style='color:#0f766e;font-weight:700'>Rs {avg_cost}</td>"
            f"<td style='width:110px'><div class='barbg'><div class='barfill' style='width:{pct}%'></div></div></td></tr>")
...
<div class='sec'><h2>🏆 Top 10 Parts — {scope}</h2><table><tr><th>Part Name</th><th>Prints</th><th>Success</th><th>Total Mat</th><th>Avg TAT</th><th>Avg Cost</th><th></th></tr>{ptab}</table></div>
