...
# Invoice ref:  eSUN ABS+  = Rs 820/kg
#               eSUN TPU-95A = Rs 1585/kg
#               eSUN PA12   = Rs 4652/kg   (+18% GST invoice me alag lagta hai)
# NOTE: PLA+ invoice me nahi tha — filhaal ABS ke barabar (820) estimate rakha
# hai; sahi rate pata chale to badal dena. Agar GST-inclusive cost chahiye to
# har number ko *1.18 karke daal do (e.g. ABS 820 -> 967.6).
# ============================================================
MATERIAL_COST_PER_KG = {
    "ABS": 820.0,
    "PLA": 820.0,    # <-- estimate, update when known
    "TPU": 1585.0,
    "PA12": 4652.0,
}
...
def dashboard():
    ...
    for r in grp:
        e=stats[(r["city"], r["printer"] or "Unknown")]
    ...
    # Avg Part Cost (Rs) + Avg TAT (turnaround) — Completed parts par, per city+machine
    ct=defaultdict(lambda:{"cost":0.0,"cnt":0,"dur":0})
    for r in db.execute(f"""SELECT city, printer, material,
            ...
            GROUP BY city, printer, material""", CITIES).fetchall():
        k=(r["city"], r["printer"] or "Unknown")
        ct[k]["cost"]+=part_cost_inr(r["matg"], r["material"])
        ct[k]["cnt"] +=r["cnt"]
        ct[k]["dur"] +=r["dur"]
    ...
    for c in CITIES:
        ...
        for key in machs:
            ...
            cc=ct[key]
            avg_cost=round(cc["cost"]/cc["cnt"]) if cc["cnt"] else 0
            avg_tat=fmt_tat(cc["dur"]/cc["cnt"]) if cc["cnt"] else "-"
            prod_rows.append({"city":city,"machine":printer,"model":e["model"],
                ...
                "avg_hours":av(e["all_h"],e["all_d"]),
                "avg_tat":avg_tat,"avg_cost":avg_cost})
    ...
    return render_template('dashboard.html', today=today_s, total=total, last_sync=last_sync,
        hours_rows=hours_rows, prod_rows=prod_rows, city_color=CITY_COLOR, cities=CITIES)
