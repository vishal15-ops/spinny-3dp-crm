...
def dashboard():
    ...
    # Avg Part Cost (Rs) + Avg TAT (turnaround) — Completed parts par, per city+machine+MATERIAL.
    # (Har material ka apna cost/time hota hai, isliye sab material milake ek avg banana
    # galat hai — is table me har material ki row alag dikhti hai.)
    ...
    for r in db.execute(f"""SELECT city, printer, material,
        ...
        k=(r["city"], r["printer"] or "Unknown", r["material"] or "Unknown")
    ...
    # Material-wise cost/TAT rows — one row per city+machine+material (only where completed parts exist)
    cost_rows=[]
    for c in CITIES:
        mkeys=sorted([k for k in ct if k[0]==c and ct[k]["cnt"]>0], key=lambda x:(x[1],x[2]))
        for key in mkeys:
            city,printer,material=key
            cc=ct[key]
            cost_rows.append({"city":city,"machine":printer,"material":material,
                "parts":cc["cnt"],
                "avg_tat":fmt_tat(cc["dur"]/cc["cnt"]),
                "avg_cost":round(cc["cost"]/cc["cnt"])})
